 ...
 * Vendored from https://github.com/AlexxIT/go2rtc (MIT License) for GH PIK2.
 *
 * Plays a go2rtc stream with the lowest possible latency:
 *   WebRTC (sub-second) -> MSE -> HLS -> MJPEG (automatic fallback).
 *
 * Usage: set `.src` to the go2rtc WebSocket URL, e.g.
 *   el.src = 'ws://localhost:1984/api/ws?src=cam1'
 * (http:// URLs are converted to ws:// automatically).
 *
 * The custom element `<go2rtc-video>` is registered at the bottom of this file.
 */
...
// Register the custom element once. `<go2rtc-video>` is a thin alias of
// VideoRTC so it can be created with document.createElement('go2rtc-video').
if (!customElements.get('go2rtc-video')) {
    customElements.define('go2rtc-video', class extends VideoRTC {});
}
